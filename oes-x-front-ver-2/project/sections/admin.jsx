  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <span style={{ flex: 1 }} />
        <button className="btn btn-primary" onClick={() => setAddDomainOpen(true)}><IconPlus size={11} /> Подключить домен</button>
      </div>
      ...
      {addDomainOpen && <AddDomainModal onClose={() => setAddDomainOpen(false)} onCreate={() => {
        if (window.notify) window.notify({ title: 'Домен подключён', body: 'Конфигурация загружена в платформу.', kind: 'success' });
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          <div className="modal-title">Подключить новый домен</div>
        ...
        <div className="modal-foot">
          ...
          <button className="btn btn-primary" disabled={!name.trim()} onClick={onCreate}><IconPlus size={11} /> Подключить</button>
        </div>
      </div>
    </div>
  );
...
function DomainCard({ d, open, onToggle }) {
  return (
...
function DomainBody({ d, compact }) {
  return (
    <div className="dom-body" style={compact ? { padding: '14px 4px 4px', gap: '20px 28px' } : null}>
      ...
      {/* apis */}
      <div className="dom-section">
        <div className="dom-section-h">
          <IconWrench size={11} /> Подключённые API · MCP
          <span style={{ flex: 1 }} />
          <button
            className="btn btn-ghost btn-sm"
            style={{ padding: '3px 8px', fontSize: 10 }}
            onClick={(e) => { e.stopPropagation(); window.notify && window.notify({ title: 'Откройте мастер MCP', body: 'Подключение нового MCP-сервера · демо.', kind: 'info' }); }}
          >
            <IconPlus size={10} /> Добавить MCP
          </button>
        </div>
        {d.apis.map(a => (
...
Object.assign(window, { SectionAdmin });
