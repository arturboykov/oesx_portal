        ...
        <div style={{ padding: '14px 18px', borderBottom: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 500, color: 'var(--fg)' }}>
              {isEdit ? `Доработка · v${editVersion} → v${nextVersion}` : 'Диалог с Workspace'}
            </div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-muted)', letterSpacing: '0.10em', textTransform: 'uppercase', marginTop: 2 }}>
              {KIND_LABEL[kind]} · {isEdit ? `REFINE-${kind.toUpperCase()}` : `BUILD-${kind.toUpperCase()}`}
            </div>
          </div>
        </div>
...
function PreviewPane({ state, kind, name, prefill }) {
  ...
  if (prefill && prefill.solutionId) {
    ...
    if (id === 'sol-01' && window.DashVolumePerShift) return <window.DashVolumePerShift />;
    if (id === 'sol-04' && window.DashCycleRating)    return <window.DashCycleRating />;
    if (id === 'sol-06' && window.DashBudget)         return <window.DashBudget />;
    if (id === 'sol-08' && window.DashLossesMonitor)  return <window.DashLossesMonitor />;
  }
  if (kind === 'dash') return <DashPreview name={name} />;
  if (kind === 'agent') return <AgentPreview name={name} />;
  if (kind === 'alert') return <AlertPreview name={name} />;
  if (kind === 'automation') return <AutomationPreview name={name} />;
  return null;
}
...
function AutomationPreview({ name }) {
  return (
    <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 'var(--r-lg)', padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
...
function defaultNameFor(kind) {
  return {
    ...
    agent: 'Q&A агент по справочнику ТО',
    alert: 'Алерт — просадка КИО ниже 75%',
    automation: 'Утренний дайджест по парку',
  }[kind] || 'Новое решение';
}
...
function buildScript(kind) {
  if (kind === 'agent') {
    return [
      { role: 'bot', text: 'Здравствуйте. Какой агент-ассистент собираем? Опишите, на какие вопросы он должен отвечать и из какого источника знаний.' },
      { role: 'user', text: 'Q&A агент по регламентам ТО экскаваторов из корпоративной базы знаний.' },
      { role: 'bot', text: 'Понял. Распознал контекст: Q&A-агент по 412 PDF из knowledge_base.maintenance + 189 документов из knowledge_base.regulations. Параметры извлечения — гибридный поиск (BM25 + векторный), MMR-перерейтинг, цитирование источника.' },
      { role: 'bot', text: 'Применил предпочтения из памяти: тон — нейтрально-инструментальный, цитаты обязательны (РД-ВГК-XX.XX), ответы на русском с английскими ID документов.' },
      { role: 'bot', text: 'Подтвердите две вещи текстом: (1) ограничивать тематику только ТО или включить НПД и стандарты тоже? (2) уровень детализации — кратко (3–5 предложений) или подробно с шагами?' },
      { role: 'user', text: 'Включить НПД. Подробно с шагами.' },
      { role: 'bot', cue: 'generating', text: 'Принято. Собираю агента — индексирую базу, генерирую тестовый диалог и встраиваю цитирование.' },
      { role: 'bot', text: 'Готово. Превью открыто в центре. Когда нажмёте «Опубликовать», агент появится в «Мои решения» как v1 и сможет отвечать в Teams/чате.' },
    ];
  }
  if (kind === 'alert') {
    return [
  ...
  if (kind === 'automation') {
    return [
      { role: 'bot', text: 'Какую автоматизацию делаем? Опишите задачу, расписание и куда отправлять результат.' },
      { role: 'user', text: 'Утренний дайджест по парку. Каждое утро в почту.' },
      ...
      { role: 'bot', text: 'Применил память: вы предпочитаете формат Mono + цветовые пороги, ночные смены тоже учитывать. Сделал заготовку шаблона письма.' },
      { role: 'bot', text: 'Уточните текстом: время отправки — 06:30 (как было раньше) или другое? Только будни или включая выходные?' },
      { role: 'user', text: '06:30, только будни.' },
      { role: 'bot', cue: 'generating', text: 'Принято. Регистрирую cron-задачу, собираю шаблон письма с цветовыми порогами и тестовую рассылку на ваш адрес.' },
      { role: 'bot', text: 'Готово. Превью письма — в центре. Опубликую как v1, расписание — пн–пт 06:30 МСК.' },
    ];
  }
  // dashboard (default)
  return [
...
Object.assign(window, { SectionCreate });
