  ...
  return (
    <nav className="nav">
      ...
      <div className="nav-right">
        <span className="nav-badge"><span className="dot dot-pos" /> Workspace · always-on</span>
        ...
        <div ref={searchRef} style={{ position: 'relative' }}>
          <button className="icon-btn" aria-label="Поиск (⌘K)" title="⌘K" onClick={() => setSearchOpen(!searchOpen)}><IconSearch size={15} /></button>
          {searchOpen && <SearchPopover onClose={() => setSearchOpen(false)} setRoute={setRoute} />}
        </div>
          ...
          <button className={`user-chip ${impersonating ? 'imp' : ''}`} onClick={() => setMenuOpen(!menuOpen)}>
            ...
            <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', lineHeight: 1.2 }}>
              ...
              <span className="urole" style={impersonating ? { color: 'var(--warn-orange)' } : null}>
                {impersonating ? 'имперсонизация · ' + me.role : me.role + (me.systemRole === 'admin' ? ' · ADMIN' : '')}
              </span>
            </span>
...
const KIND_OPTIONS = [
  ...
  { id: 'agent',      label: 'Агент-ассистент', icon: IconBrain,       desc: 'Q&A по базе знаний или данным' },
  { id: 'alert',      label: 'Алерт',           icon: IconAlertCircle, desc: 'Уведомление по порогу' },
  { id: 'automation', label: 'Автоматизация',   icon: IconZap,         desc: 'Регулярная задача по cron' },
];
...
function Sidebar({ route, setRoute, role, openCreate, collapsed, onToggleCollapse }) {
  ...
  return (
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: 16 }}>
        <SidebarLimit collapsed={collapsed} />
      </div>
    </aside>
  );
}
...
function SidebarLimit({ collapsed }) {
  const t = window.__tweaks || { limitState: 'normal' };
  const limitData = useLimitState(t.limitState);
...
Object.assign(window, { TopNav, Sidebar, ROUTE_DEFS, KIND_OPTIONS, CreateButton, useLimitState });
